...
19. As a job seeker, I want the original scraped or pasted raw text to be stored alongside the structured data, so that I can reference the full JD later.
20. As a job seeker, I want to see a loading state while the app is scraping and extracting data, so that I know the process is in progress.

### AI-Powered JD Extraction
21. As a job seeker, I want the AI to automatically extract the job title from the JD, so that I don't have to type it manually.
22. As a job seeker, I want the AI to extract the company name from the JD, so that it is saved accurately.
23. As a job seeker, I want the AI to extract the job location and job type (remote, hybrid, on-site), so that I can filter applications by location.
24. As a job seeker, I want the AI to extract the salary range when mentioned in the JD, so that I can compare compensation across applications.
25. As a job seeker, I want the AI to extract required skills as a structured list, so that I can see exactly what the role demands.
26. As a job seeker, I want the AI to extract nice-to-have skills separately from required skills, so that I can prioritize learning gaps.
27. As a job seeker, I want the AI to extract years of experience required, so that I can assess my fit at a glance.
28. As a job seeker, I want the AI to extract key responsibilities as a bullet list, so that I understand the day-to-day of the role.
29. As a job seeker, I want the AI to extract a short "About the Company" section when present, so that I have context for interviews.
30. As a job seeker, I want the AI to calculate a Match Score (0–100%) comparing the JD requirements to my profile, so that I can prioritize the best-fit applications.
31. As a job seeker, I want to review and correct any AI-extracted fields before saving, so that errors from extraction don't persist in my data.

### Application Pipeline Tracking
32. As a job seeker, I want every saved job to start at "Saved" status, so that there is a clear starting point for each application.
33. As a job seeker, I want to move a job application to "Applied" with the application date, so that I track when I submitted.
34. As a job seeker, I want to progress an application through Phone Screen, Interview, and Offer stages, so that the pipeline reflects reality.
35. As a job seeker, I want to mark an application as Accepted, Rejected, or Withdrawn, so that the pipeline reaches a final state.
36. As a job seeker, I want to add timestamped notes to each application, so that I can log interview feedback, recruiter conversations, and next steps.
37. As a job seeker, I want to set a follow-up date on each application, so that I receive a visual reminder on the dashboard when one is due.
38. As a job seeker, I want to view my applications as a sortable and filterable list/table, so that I can quickly find a specific application.
39. As a job seeker, I want to toggle between a list view and a Kanban board view for my applications, so that I can visualize the pipeline.
40. As a job seeker, I want to filter applications by status, company, location, or date added, so that I can focus on a subset of applications.

### Dashboard
41. As a job seeker, I want to see a total count of my applications on the dashboard, so that I have a high-level overview.
42. As a job seeker, I want to see a breakdown of applications by pipeline stage on the dashboard, so that I can spot bottlenecks.
43. As a job seeker, I want to see a chart of applications added over time, so that I can measure the pace of my job search.
44. As a job seeker, I want to see a list of applications with upcoming follow-up dates on the dashboard, so that I never miss a deadline.
45. As a job seeker, I want to see my recent activity on the dashboard, so that I can resume where I left off.
46. As a job seeker, I want to see my average Match Score across all applications, so that I can gauge overall fit quality.

### CV Generation
47. As a job seeker, I want to trigger AI-powered CV generation for a specific job application, so that I get a version tailored to that JD.
48. As a job seeker, I want to choose from 3 CV templates (Modern, Classic, Minimal) before generating, so that the output matches my preferred style.
49. As a job seeker, I want to see a live preview of the generated CV in the browser, so that I can review it before downloading.
50. As a job seeker, I want the AI to rewrite my professional summary to match the specific JD, so that the CV feels targeted rather than generic.
51. As a job seeker, I want the AI to reorder and highlight the most relevant skills based on the JD's required skills, so that the recruiter sees the best match immediately.
52. As a job seeker, I want the AI to select and reframe my most relevant work experience entries for each JD, so that the CV is concise and compelling.
53. As a job seeker, I want the AI to include relevant projects from my profile when they strengthen the application, so that my portfolio is leveraged.
54. As a job seeker, I want to download the generated CV as a PDF, so that I can attach it to job applications.
55. As a job seeker, I want previously generated CVs for a job to be stored in the job detail view, so that I can re-download them without regenerating.
56. As a job seeker, I want to regenerate a CV with a different template without losing the previous version, so that I have options.

### Cover Letter Generation
57. As a job seeker, I want the AI to generate a personalized cover letter for each job application, so that I have a strong written accompaniment to my CV.
58. As a job seeker, I want the cover letter to reference specific details from the JD (company name, role, key requirements), so that it feels genuinely tailored.
59. As a job seeker, I want to edit the AI-generated cover letter inline before exporting, so that I can add my personal voice.
60. As a job seeker, I want to export the cover letter as a PDF, so that I can attach it alongside my CV.
61. As a job seeker, I want to copy the cover letter as plain text to the clipboard, so that I can paste it into online application forms.
62. As a job seeker, I want previously generated cover letters to be stored per job application, so that I can revisit them.

### AI Provider & Settings
63. As a job seeker, I want to choose my preferred AI provider from a list (Gemini, OpenAI, Anthropic, Groq), so that I can use the model I have access to.
64. As a job seeker, I want to enter and save my own API key for each provider, so that my usage costs are under my own account.
65. As a job seeker, I want my API keys to be stored securely (encrypted at rest in Supabase), so that they are not exposed.
66. As a job seeker, I want to select which specific model to use within a provider (e.g., gpt-4o vs gpt-4o-mini), so that I can balance speed and quality.
67. As a job seeker, I want to test my API key connection before saving, so that I know it works before attempting generation.
68. As a job seeker, I want to switch AI providers at any time, so that I am not locked into one service.

### Design & UX
69. As a job seeker, I want to toggle between light and dark mode, so that I can use the app comfortably in any environment.
70. As a job seeker, I want the app to be fully responsive on mobile, so that I can manage applications from my phone.
71. As a job seeker, I want smooth transitions and micro-animations throughout the UI, so that the experience feels polished and premium.
72. As a job seeker, I want status badges to be color-coded by pipeline stage, so that I can identify application statuses at a glance.
...
#### 4. AI Extraction Module (Deep Module)
...
#### 5. AI Generation Module (Deep Module)
...
- Browser extension to capture jobs from any site without URLs
